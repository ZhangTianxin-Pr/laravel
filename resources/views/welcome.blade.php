...
<div class="container">
    <div class="row">        
        ...
        <div class="col-sm-4">
            <form class="form-inline" method="post" name="input_search" action="{{ route('InputSearch') }}" id="input_search">
                {{ csrf_field() }}
                <label class="sr-only input-research" for="search1">input-research</label>
                <input type="text"  style="width:48%; height:35px;float: left" class="form-control input-research-text" id="search1" placeholder="PMID/GSM/SRA" name="input_search">
                <span>&nbsp;</span>
                <input type="submit"  style="width:50%; height:35px;" class="btn btn-default input-research-button" value="Search" >
            </form>
        </div>
    </div>
    <div class="row">
...
<div class="container">
    <div class="row">        
        <div class="col-sm-12">
            <div class="table-responsive">
                <table class="table table-hover table-expandable">
                    ...
                    @foreach($totals as $total)
                       <tr class="tr-style">
                           <td>{{$total->SRXID}}</td>
                           <td>{{$total->Factor_relation->FACTOR ?? ''}}</td>
                           <td>{{$total->Tissue_relation->TISSUE ?? ''}}</td>
                           <td>{{$total->Cellline_relation->CELL_LINE_CON ?? ''}}</td>
                           <td>{{$total->Stage}}</td>
                       </tr>
                       <tr  style="display: none;" class="tr-style">
                           <td colspan="8">
                               <ul>
                                   @if (count($total->Sex) > 0 )
                                   <li>Factor Known As : {{$total->Factor_relation->FACTOR_KNOWN_AS ?? ''}}</li>
                                   @else
                                   ...
                                   <li>Sex : {{$total->Sex}} </li>
                                   @else
                                       <li style="display: none;">Sex : {{$total->Sex}} </li>
                                   @endif
                                   @if (count($total->GSMID) > 0 )
                                   <li>GSMID : {{$total->GSMID}} </li>
                                   @else
                                       <li style="display: none;">GSMID : {{$total->GSMID}}</li>
                                   @endif
                                   @if (count($total->Antibody) > 0 )
                                   <li>Antibody : {{$total->Antibody}} </li>
                                   @else
                                       <li style="display: none;">Antibody : {{$total->Antibody}} </li>
                                   @endif
                                   @if (count($total->Genotype) > 0 )
                                   <li>Genotype : {{$total->Genotype}} </li>
                                   @else
                                       <li style="display: none;">Genotype : {{$total->Genotype}}  </li>
                                   @endif
                                   @if (count($total->Cia_relation) > 0 )
                                   <li>Citation :
                                       @foreach($total->Cia_relation as $information)
                                   ...
                                   <li><a href="#name1" class="tr-button-style">View the Results of the Motif and Target</a></li>
                                       <li>
                                           <table class="table table-bordered">
                                               ...
                                               <tr class="tr-style">
                                                   @if($total->FastQC>=25)
                                                       <td class="font-color2">{{$total->FastQC}}</td>
                                                   @else
                                                       <td class="font-color1">{{$total->FastQC}}</td>
                                                   @endif

                                                   @if($total->Uniquely_Mapped>=60)
                                                       <td class="font-color2">{{$total->Uniquely_Mapped}}%</td>
                                                   @else
                                                       <td class="font-color1">{{$total->Uniquely_Mapped}}%</td>
                                                   @endif
                                                   @if($total->FRip>=1)
                                                       <td class="font-color2">{{$total->FRip}}%</td>
                                                   @else
                                                       <td class="font-color1">{{$total->FRip}}%</td>
                                                   @endif
                                                   @if($total->FC_10>=20)
                                                       <td class="font-color2">{{$total->FC_10}}%</td>
                                                   @else
                                                       <td class="font-color1">{{$total->FC_10}}%</td>
                                                   @endif

                                                   @if($total->DHS>=70)
                                                       <td class="font-color2">{{$total->DHS}}%</td>
                                                   @else
                                                       <td class="font-color1">{{$total->DHS}}%</td>
                                                   @endif
                                                   @if($total->PBC>=80)
                                                       <td class="font-color2">{{$total->PBC}}%</td>
                                                   @else
                                                       <td class="font-color1">{{$total->PBC}}%</td>
                                                   @endif
                                                   <td>{{$total->Peak}}</td>
                                               </tr>
                                               </tbody>
            ...
            {{ $totals->appends(request()->all())->links() }}
        </div>
    </div>
</div>
